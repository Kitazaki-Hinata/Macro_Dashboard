...
from gui.bbg_browser import BloombergBrowser, validate_article_dict
...
class BloombergExtractor:
    ...
    def create_driver(self) -> BloombergBrowser:
        # 保留原调用接口，浏览器由系统打开，JSON 由本机扩展传回。
        return BloombergBrowser()

    def _fetch_bbg_article(self, driver : BloombergBrowser):
        """
        ...
        try:
            ...
            article_dict : dict = driver.fetch_article(self.bbg_article_url)
            validate_article_dict(article_dict)
            return True, article_dict

        except Exception as e:
            return False, {"error" : f'Failed to fetch, reason is {e}'}

    def edit_bbg_article(self, driver : BloombergBrowser) -> Tuple[bool, str]:
