...
class _DownloadWorker(QObject):
    ...
    def __init__(
        ...
        start_year: int,
        download_all: bool,
        selected_sources: Optional[list[str]] | None = None,
        main_window: Optional[_MainWindowProto] = None,
    ):
        super().__init__()
...
class UiFunctions:  # 删除:mainWindow
    ...
    def on_connect_charts_changed(self, state):
        ...
        try:
            if hasattr(self.main_window, "chart_functions") and hasattr(
                self.main_window.chart_functions, "toggle_four_charts_link"
            ):
                ...
                def sync_mouse(pos):
                    ...
                    for w in widgets:
                        try:
                            ...
                            cross = (
                                getattr(
                                    self.main_window.chart_functions, "crosshairs", {}
                                )
    ...
    def clear_logs(self):
        ...
        try:
            if (
                hasattr(self.main_window, "console_area")
                and self.main_window.console_area is not None
            ):
    ...
    def note_rename_page(self):
        ...
        if (
            note_name == "note_instructions_btn"
        ...
        for i in range(layout.count()):
            ...
            if item and item.widget():
                ...
                if isinstance(widget, QPushButton) and widget.text() == note_name:
                    ...
                    def finish_rename(
                        le: QLineEdit = line_edit,
                        btn: QPushButton = widget,
                        old_name: str = old_object_name,
                        idx: int = i,
                    ):
                            ...
                            if is_valid and new_name != "note_instructions_btn":
                                ...
                                for j in range(layout.count()):
                                    ...
                                    if item_check and item_check.widget():
                                        ...
                                        if (
                                            isinstance(check_widget, QPushButton)
                                            and check_widget.text() == new_name
                                        ):
                                            duplicate = True
                                            break
                                ...
                                if not duplicate:
                                    ...
                                    parent_dir = os.path.dirname(current_dir)
                                    note_dir = os.path.join(parent_dir, "note")
                                    origin_txt_file_path = os.path.join(
                                        note_dir, f"{old_name}.txt"
                                    )
                                    changed_txt_file_path = os.path.join(
                                        note_dir, f"{new_name}.txt"
                                    )

                                    if os.path.exists(origin_txt_file_path):
                                        os.rename(
                                            origin_txt_file_path, changed_txt_file_path
                                        )
                                    ...
                                    # 重新连接新的信号
                                    btn.clicked.connect(
                                        lambda checked: self.note_btn_open_file_slot(
                                            new_name
                                        )
                                    )  # type: ignore
    ...
    def open_settings_window(self, ui: Any, window: QWidget, name: str):
        ...
        if name == "one":
            try:
                ...
                ui.first_data_selection_box.setCurrentText(
                    existing_data["one_chart_settings"]["first_data"]["data_name"]
                )
                ui.second_data_selection_box.setCurrentText(
                    existing_data["one_chart_settings"]["second_data"]["data_name"]
                )
            except:
                pass
        ...
        if name == "four":
            try:
                existing_data: Dict[str, Any] = self.get_settings_from_json()
                ui.first_data_selection_box.setCurrentText(
                    existing_data["four_chart_settings"]["first_data"]["data_name"]
                )
                ui.second_data_selection_box.setCurrentText(
                    existing_data["four_chart_settings"]["second_data"]["data_name"]
                )
                ui.third_data_selection_box.setCurrentText(
                    existing_data["four_chart_settings"]["third_data"]["data_name"]
                )
                ui.fourth_data_selection_box.setCurrentText(
                    existing_data["four_chart_settings"]["fourth_data"]["data_name"]
                )
            except:
                pass
    ...
    def one_finish_settings(self, window: Any, widget: QWidget):
        ...
        if isinstance(second_data, str) and second_data.strip():
            existing_data["one_chart_settings"]["second_data"][
                "data_name"
            ] = second_data
        ...
        main_plot_widget = self.main_window.graph_widget_2.findChild(
            pg.PlotWidget, "main_plot_widget"
        )
        if main_plot_widget is not None:
            ...
            try:
                ...
                keep = set()
                try:
                    v_line, h_line = self.main_window.chart_functions.crosshairs.get(
                        "main_plot_widget", (None, None)
                    )
                except Exception:
                    v_line = h_line = None
                lbl = None
                try:
                    lbl = self.main_window.chart_functions.labels.get(
                        "main_plot_widget"
                    )
                except Exception:
                    pass
            ...
            if isinstance(second_data, str) and second_data.strip():
                # 第二个数据
                dates, values = (
                    self.main_window.chart_functions._get_data_from_database(
                        second_data
                    )
                )
                ...
                def _rescale_right_y():
                    ...
                    try:
                        ...
                        last_xrng = getattr(
                            main_plot_widget, "_right_last_xrange", None
                        )
                        ...
                        last_yrng = getattr(
                            main_plot_widget, "_right_last_yrange", None
                        )
                        # 阈值：若变化幅度很小则不更新，减少抖动
                        if last_yrng is not None:
                            ...
                            if (
                                rel_diff(v_min, old_min) < 0.01
                                and rel_diff(v_max, old_max) < 0.01
                            ):
                                setattr(
                                    main_plot_widget, "_right_last_xrange", cur_xrng
                                )
                                return
                ...
                try:
                    second_curve.setData(
                        x=flash_date_buff, y=values, pen=pen, name=second_data
                    )
                except Exception:
        ...
        try:
            ...
            try:
                ...
                main_curves = [
                    it
                    for it in plot_item.listDataItems()
                    if getattr(it, "opts", {}).get("name")
                ]
                added = set()
                ...
                if hasattr(main_plot_widget, "_right_viewbox") and getattr(main_plot_widget, "_right_viewbox") is not None:  # type: ignore[attr-defined]
                    ...
                    for it in list(getattr(rvb, "addedItems", [])):
                        nm = (
                            getattr(getattr(it, "opts", {}), "get", lambda *_: None)(
                                "name"
                            )
                            if hasattr(it, "opts")
                            else None
    ...
    def four_finish_settings(self, window: Any, widget: QWidget):
        ...
        colors = []
        for btn in [
            window.first_color_btn,
            window.second_color_btn,
            window.third_color_btn,
            window.fourth_color_btn,
        ]:
            try:
        ...
        for key, name, color in zip(
            ["first_data", "second_data", "third_data", "fourth_data"], names, colors
        ):
            try:
        ...
        mapping = [
            (
                self.main_window.four_chart_one,
                "four_chart_one_plot",
                names[0],
                colors[0],
            ),
            (
                self.main_window.four_chart_two,
                "four_chart_two_plot",
                names[1],
                colors[1],
            ),
            (
                self.main_window.four_chart_three,
                "four_chart_three_plot",
                names[2],
                colors[2],
            ),
            (
                self.main_window.four_chart_four,
                "four_chart_four_plot",
                names[3],
                colors[3],
            ),
        ]
    ...
    def _load_request_json(self) -> Optional[Dict[str, Any]]:
        try:
            req_path = os.path.join(
                os.path.abspath(os.path.dirname(__file__)), "..", "request_id.json"
            )
    ...
    def start_download(self):
        ...
        if parallel:
            self._append_console(
                f"Start parallel download ({len(sources)} sources, threads={max_threads}) from year {start_year}..."
            )
            # 使用标准的 Qt 线程模式处理并行下载
    ...
    def _cleanup_thread(self):
        ...
        worker = self._worker
        was_cancelled = (
            bool(getattr(worker, "_is_cancelled", False)) if worker else False
        )
        ...
        self.main_window.table_update_label.setText(
            f"Recent Update Time : {date_today}"
        )
