...
def _exponential_backoff_delays(
    max_attempts: int, base: float = 0.5, factor: float = 2.0, jitter: float = 0.25
) -> List[float]:
    """生成指数退避延时序列（带抖动）。
    ...
    for i, delay in enumerate(delays, start=1):
        try:
            ...
            if (
                ...
                and int(status) in {400, 401, 403, 404, 405, 406, 410, 422}
            ):
...
def http_post_with_retry(
    ...
    *,
    ...
    delays = (
        ...
        else _exponential_backoff_delays(max_attempts)
    )
    last_exc: Optional[Exception] = None
    for i, delay in enumerate(delays, start=1):
        try:
            ...
            status = getattr(resp, "status_code", None)
            logger.info(
                "HTTP POST %s attempt=%d status=%s in %.3fs", url, i, status, dt
            )
            if resp.ok:
                return resp
            # 对常见非瞬时 4xx 错误不重试，直接失败
        ...
        if i < max_attempts:
            try:
                logger.info(
                    "Retrying POST in %.1fs (attempt %d/%d)",
                    float(delay),
                    i + 1,
                    max_attempts,
                )
...
def yf_download_with_retry(
    symbol: str, *, start: str, end: str, interval: str = "1d", max_attempts: int = 5
) -> pd.DataFrame:
    ...
    """
    delays = _exponential_backoff_delays(
        max_attempts=max_attempts, base=1.0, factor=2.0, jitter=0.5
    )
    ...
    raise Exception(
        f"yfinance download failed for {symbol} after {max_attempts} attempts: {last_err}"
    )
...
class DatabaseConverter:
    ...
    @staticmethod
    def _format_converter(
        df: Optional[pd.DataFrame], data_name: str, is_pct_data: bool
    ) -> pd.DataFrame:
        ...
        def finalize_with_date_first(df_in: pd.DataFrame) -> pd.DataFrame:
            ...
            df_in = df_in.dropna(subset=["date"]).drop_duplicates(
        ...
        logger.debug(
            "_format_converter start: data=%s empty=%s columns=%s index_name=%s shape=%s",
            data_name,
            df.empty,
            list(df.columns),
        ...
        try:
            ...
            if re.fullmatch(r"\d{4}", sample):
                logger.debug(
                    "_format_converter matched BEA annual for %s (sample=%s)",
                    data_name,
                    sample,
                )
                # 年度：用当年末一天
            ...
            if re.fullmatch(r"\d{4}Q[1-4]", sample):
                logger.debug(
                    "_format_converter matched BEA quarterly for %s (sample=%s)",
                    data_name,
                    sample,
                )
            ...
            if re.fullmatch(r"\d{4}M\d{2}", sample):
                logger.debug(
                    "_format_converter matched BEA monthly for %s (sample=%s)",
                    data_name,
                    sample,
                )
        ...
        try:
            ...
            if set(df.columns) >= ohlcv:
                ...
                # 仅保留 Close => data_name
                out = pd.DataFrame({"date": df["date"], data_name: df["Close"]})
        ...
        try:
            ...
            if cols == ["year", "period", "value"] or cols == [
                "year",
                "period",
                "MoM_growth",
            ]:
                ...
                if period.str.startswith("M").any():
                    ...
                    logger.debug(
                        "_format_converter BLS quarterly period detected for %s",
                        data_name,
                    )
                ...
                dt = pd.to_datetime(
                    pd.DataFrame({"year": year_num, "month": month_num, "day": 1}),
                    errors="coerce",
                )
        ...
        try:
            if (
                "date" in df.columns
                and df["date"].astype(str).str.contains(r"^[A-Za-z]{3}_[0-9]{4}$").any()
            ):
                ...
                dt = pd.to_datetime(
                    pd.DataFrame({"year": year, "month": month_num, "day": 1}),
                    errors="coerce",
                )
                df["date"] = dt.dt.strftime("%Y-%m-%d")
        ...
        try:
            if "date" in df.columns:
                logger.debug(
                    "_format_converter fallback by date column for %s", data_name
                )
                return finalize_with_date_first(
                    df.rename(columns={df.columns[1]: data_name})
                    if len(df.columns) > 1
                    else df
                )
            else:
                logger.debug(
                    "_format_converter fallback by index->date for %s", data_name
                )
                dates = pd.to_datetime(
                    pd.Series(df.index).astype(str), errors="coerce"
                ).dt.strftime("%Y-%m-%d")
    ...
    def _create_ts_sheet(self, start_date: str) -> sqlite3.Cursor:
        ...
        try:
            if not table_exists:
                t0 = time.perf_counter()
                cursor.execute(
                    "CREATE TABLE IF NOT EXISTS Time_Series(date DATE PRIMARY KEY)"
                )
                current_date = datetime.strptime(start_date, "%Y-%m-%d").date()
                while (
                    current_date <= DatabaseConverter.end_date
                ):  # 直接比较date对象   # 添加时间列，循环写入从设置的开始日期一直到今天的所有日期
                    cursor.execute(
                ...
                self.conn.commit()
                logger.info(
                    "Time_Series table created and initialized (%.3fs)",
                ...
                if start_date_obj < min_date:
                    ...
                    self.conn.commit()
                    logger.info(
                        "Time_Series table prepended %d earlier date rows (%.3fs)",
                        len(dates_to_prepend),
                        time.perf_counter() - t0_back,
                    )
                ...
                if current_date > max_date:
                    ...
                    self.conn.commit()
                    logger.info(
                        "Time_Series table appended %d future date rows (%.3fs)",
                        len(dates_to_add),
    ...
    def _ensure_ts_primary_key(self) -> None:
        ...
        try:
            ...
            if has_pk:
                return
            logger.warning(
                "Time_Series missing PRIMARY KEY on 'date' – rebuilding to restore constraint"
            )
            # 收集现有列
            existing_cols: List[str] = [row[1] for row in cols_info]
            if "date" not in existing_cols:
                logger.error(
                    "Time_Series table unexpectedly lacks 'date' column, skip pk repair"
                )
                return
            # 生成列类型（保留原类型，缺省使用 REAL）
            col_type_map: Dict[str, str] = {
                row[1]: (row[2] or "REAL") for row in cols_info
            }
            ...
            self.conn.commit()
            logger.info(
                "Time_Series primary key restored; columns=%s", ["date"] + data_cols
            )
        except Exception as e:
    ...
    def write_into_db(
        ...
        with DB_WRITE_LOCK:
            ...
            try:
                logger.info(
                    "write_into_db start: data=%s, is_time_series=%s, shape=%s",
                    data_name,
                    is_time_series,
                    tuple(df.shape),
                )
                if df.empty:
                    logger.error(
                        f"{data_name} is empty, FAILED INSERT, locate in write_into_db"
                    )
                else:
                    if is_time_series:  # 时间序列进入增量模式
                        t0 = time.perf_counter()
                        df_fmt: pd.DataFrame = DatabaseConverter._format_converter(
                            df, data_name, is_pct_data
                        )
                        logger.debug(
                            "%s after format: columns=%s, shape=%s",
                            data_name,
                            list(df_fmt.columns),
                            tuple(df_fmt.shape),
                        )
                        if df_fmt.empty or "date" not in df_fmt.columns:
                            logger.error(
                                "%s reformat produced empty/invalid dataframe, skip writing",
                                data_name,
                            )
                            return
                        df_fmt = df_fmt.copy()
                        df_fmt["date"] = pd.to_datetime(df_fmt["date"], errors="coerce")
                        df_fmt = df_fmt.dropna(subset=["date"]).drop_duplicates(
                            subset=["date"], keep="last"
                        )
                        ...
                        all_dates = pd.date_range(
                            start=start_date, end=date.today(), freq="D"
                        )
                        ...
                        if df_full[data_name].notna().any():
                            ...
                            if pd.isna(df_full.iloc[0][data_name]):
                                first_valid_value = df_full.iloc[first_valid_pos][
                                    data_name
                                ]
                                df_full.loc[
                                    df_full.index[: first_valid_pos + 1], data_name
                                ] = first_valid_value
                        ...
                        if not re.fullmatch(r"[A-Za-z0-9_]+", data_name):
                            logger.error(
                                "Invalid column name '%s', abort writing", data_name
                            )
                            return
                        # 若列不存在则添加
                        try:
                            cursor.execute(
                                f"ALTER TABLE Time_Series ADD COLUMN {data_name} REAL"
                            )
                        ...
                        if (not overwrite_existing) or only_fill_null:
                            try:
                                cursor.execute(
                                    f"SELECT date, {data_name} FROM Time_Series"
                                )
                        ...
                        for date_str, v in df_full[["date", data_name]].itertuples(
                        ...
                        logger.debug(
                            "%s returns 2 cols shape=%s", data_name, tuple(rtn_df.shape)
                        )
                        logger.info(
                            "write_into_db finished: data=%s (%.3fs)",
                            data_name,
                        ...
                        if not exists:
                            df.to_sql(
                                data_name, self.conn, if_exists="fail", index=False
                            )
                            self.conn.commit()
                            logger.info(
                                "write_into_db(sheet=%s): created table rows=%d",
                                data_name,
                                len(df),
                            )
                        else:
                            if has_date:
                                ...
                                if not has_pk:
                                    ...
                                    cursor.execute(
                                        f"ALTER TABLE {data_name} RENAME TO {data_name}_backup"
                                    )
                                    col_defs = ["date TEXT PRIMARY KEY"] + [
                                        f"{c} REAL" for c in tmp_cols if c != "date"
                                    ]
                                    cursor.execute(
                                        f"CREATE TABLE {data_name} ({', '.join(col_defs)})"
                                    )
                                    cols_sel = ",".join(tmp_cols)
                                    cursor.execute(
                                        f"INSERT OR REPLACE INTO {data_name} ({cols_sel}) SELECT {cols_sel} FROM {data_name}_backup"
                                    )
                                ...
                                self.conn.commit()
                                logger.info(
                                    "write_into_db(sheet=%s): upserted %d rows",
                                    data_name,
                                    len(rows),
                                )
                            else:
                                ...
                                self.conn.commit()
                                logger.warning(
                                    "write_into_db(sheet=%s): appended %d rows (no date pk)",
                                    data_name,
                                    len(df),
                                )
                        logger.info(
                ...
                logger.error(
                    f"FAILED to write into database, in method write_into_db, since {e}"
                )
                print(f"error {e}")
                return
...
class BEADownloader(DataDownloader):
    ...
    def __init__(
        ...
        self.time_range_lag: str = self.time_range[
            :-5
        ]  # 去除最后5个字符，即去除最后一年
    ...
    def to_db(
        ...
        def worker(
            ...
            try:
                ...
                logger.info(
                    f"BEA_{table_name} Successfully extracted! rows={len(df_modified)}"
                )

                if df_modified.empty:
                    logging.error(
                        f"{table_name} is empty, FAILED INSERT, locate in to_db"
                    )
                ...
                logger.error(
                    f"{table_name} FAILED DOWNLOAD/REFORMAT in BEA worker: {e}"
                )
                return table_name, None
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:  # 创建一个线程池
            future_map = {
                ex.submit(worker, tn, cfg): tn for tn, cfg in items
            }  # 往线程池里面添加submit，每个submit添加worker函数用于下载，tn数据名称，cfg是json的参数
                    ...
                    if return_csv:
                        for name, df in df_dict.items():
                            try:
                                ...
                                logging.info(
                                    f"{name} saved to {csv_path} Successfully!"
                                )
                            except Exception as e:
                                logging.error(
                                    f"{name} FAILED DOWNLOAD CSV in method 'to_csv', since {e}"
                                )
                                continue
                except Exception as e:
                ...
                if return_csv:
                    for name, df in df_dict.items():
                        try:
                            ...
                            logging.error(
                                f"{name} FAILED DOWNLOAD CSV in method 'to_db', since {e}"
                            )
                            continue
            except Exception as e:
        ...
        if bls_debug:
            ...
            def worker(
                ...
                try:
                    ...
                    logger.error(
                        f"{table_name} FAILED EXTRACT DATA from BLS, probably due to API or network issues: {e}"
                    )
                    return table_name, None
                ...
                try:
                    ...
                    try:
                        ...
                        logger.warning(
                            f"{table_name} FAILED REFORMAT: DROP USELESS COLUMNS, continue"
                        )
                    except Exception as e:
                        logger.error(
                            f"{table_name} FAILED REFORMAT data from BLS, errors in df managing, {e}"
                        )
                        return table_name, None
                ...
                if table_config["needs_pct"] is True:
                    try:
                        ...
                        logger.error(
                            f"{table_name} FAILED REFORMAT PERCENTAGE, probably due to df error, {e}"
                        )
                        return table_name, None
...
class TEDownloader(DataDownloader):
    ...
    def _get_data_from_trading_economics_month(
        ...
        try:
            ...
            try:
                ...
                logging.error(
                    f"{data_name} FAILED TO CLICK 'bar chart type' button, {e}"
                )
                return None
        except Exception as e:
        ...
        try:
            ...
            logging.error(
                f"{data_name} FAILED TO EXTRACT data from html, but successfully get data from website, {e}"
            )
            return None
    ...
    def to_db(
        ...
        for table_name, table_config in self.json_dict.items():
            ...
            if df is None:
                logging.error(
                    f"FAILED TO EXTRACT {table_name}, check PREVIOUS loggings"
                )
                continue
            converter = DatabaseConverter()
            converter.write_into_db(
...
class DownloaderFactory:
    ...
    def _get_api_key(cls, source: str) -> Optional[str]:
        ...
        if not api:
            logging.warning(
                "_get_api_key WARNING: API key NOT FOUND in .env file, CONTINUE"
            )
            return ""
        return api
    ...
    def create_downloader(
        ...
        return downloader_classes[source](
            json_dict=cfg, api_key=api_key, request_year=request_year
        )
